...
for STACK in "${STACKS[@]}"; do
  ...
  # 上限を超える場合は末尾を切り詰める。
  # NOTE: 以前は `printf ... | head -c` を使っていたが、set -o pipefail 下では
  #       head が先に stdin を閉じることで printf が SIGPIPE(終了コード141)となり、
  #       代入が失敗 → set -e で無言終了する不具合があった。
  #       外部プロセスを使わない bash の部分文字列展開に変更(文字数基準にも整合)。
...
# NOTE: `grep ... | head -1` も pipefail 下で grep が SIGPIPE になり得るため、
#       grep -m1(最初の一致で grep 自身が終了)に変更している。
